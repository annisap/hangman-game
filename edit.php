...

{
   ...
   // if(!empty($english) || !empty) {

        $result = $dbc->query("UPDATE word set word_en = '$english', lower='$lower', word_gr='$greek', synonym='$synonym' WHERE id_w =
'$id'") or die("Query error: " . $dbc->error);
...
{
    $result = $dbc->query("SELECT id_w, word_en, lower, word_gr, synonym FROM word WHERE id_w = '{$_GET['id']}'");
                        ...
                        <form action="" method="POST" class="form-insert">
                            ...
                            <div class="input-group input-group-lg">
                                ...
                                <input type="text" class="form-control" placeholder="Enter the word" name="english"
                                       aria-describedby="eng" maxlength="20" value="<?=$engl;?>">
                            </div>
                            <div class="input-group input-group-lg">
                                <span class="input-group-addon" id="level">LEVEL</span>
                                <select class="form-control" name="lower" aria-describedby="level">
                                    <option value="1" <? if ($level==1) echo 'selected="selected"';?>>Elementary
                                    </option>
                                    <option value="0" <? if ($level==0) echo 'selected="selected"';?>">Advanced
                                    </option>
                                </select>
                            ...
                            <div class="input-group input-group-lg">
                                ...
                                <input type="text" class="form-control" placeholder="Enter the word" name="greek"
                                       aria-describedby="gr" maxlength="20" value="<?=$gr;?>">
                            </div>
                            ...
                            <div class="input-group input-group-lg">
                                ...
                                <input type="text" class="form-control" placeholder="Enter the synonym" name="synonym"
                                       aria-describedby="synonym" maxlength="50" value="<?=$syn;?>">
                            </div>
